...
protected:
	 ...
	 * \return  1 : Parity check was satisfied.
	 */
	int Check_Node_Split_Row(Buffer<int> &in_out_msg, float esf_factor, unsigned int num_partitions, unsigned int threshold);
    
    void Distribute_Messages_Partitions(std::vector< std::vector<int> > &parts, Buffer<int> &in_out_msg, unsigned int partitions, unsigned int len);
...
//    bool Neighbors_Threshold_Enable(unsigned int part_num, unsigned int partitions, unsigned int treshold_en[]);
    ...
    void Regroup_Messages_Partitions(std::vector< std::vector<int> > &parts, Buffer<int> &in_out_msg, unsigned int len);
