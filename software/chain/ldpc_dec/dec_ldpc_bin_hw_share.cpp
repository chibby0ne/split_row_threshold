                                                           ...
                                                           Buffer<int>  &check_node_out)
{
	...
	for(unsigned int cn2vn_msg = 0; cn2vn_msg < max_check_degree_; cn2vn_msg++)
	{
		...
		if(vng_select > -1)
		{
            ...
            // tempAPP = lambda channel + epsilon of that iteration
			current_app_value += current_message;

            // if it's not the first iteration
            // we must substract epsilon of previous iteration => APP(i) = appchannel + epsilon(i) where i = iteration
			if (iter != 0)
...
int Decoder_LDPC_Binary_HW_Share::Check_Node_Split_Row(Buffer<int> &in_out_msg, float esf_factor, unsigned int partitions, unsigned int threshold)
{
...
void Decoder_LDPC_Binary_HW_Share::Distribute_Messages_Partitions(std::vector< std::vector<int> > &parts, Buffer<int> &in_out_msg, unsigned int partitions, unsigned int len)
{
...
void Decoder_LDPC_Binary_HW_Share::Multiply_Sign_Esf(std::vector<int>& part, std::vector<unsigned int>& temp_part, int local_pchk_sign, float esf_factor, unsigned int part_num)
{
...
void Decoder_LDPC_Binary_HW_Share::Regroup_Messages_Partitions(std::vector< std::vector<int> > &parts, Buffer<int> &in_out_msg, unsigned int len)
{
...
unsigned int Decoder_LDPC_Binary_HW_Share::Minimum_Fixp_Split_Row(Buffer<unsigned int> &message, unsigned int& idx, unsigned int part_num, unsigned int check_node_degree)
{
